...
@st.cache_data
def load_data(dataset, epsilon):
    """Loads and caches data, checking multiple filename cases to prevent 'Not Found' errors."""
    eps_str = epsilon.replace("/", "_")
    
    # Try multiple casing formats (e.g., 'imagenette', 'Imagenette', 'IMAGENETTE')
    filenames_to_try = [
        f"Evals_{dataset.lower()}_{eps_str}.xlsx",
        f"Evals_{dataset.capitalize()}_{eps_str}.xlsx",
        f"Evals_{dataset.upper()}_{eps_str}.xlsx",
        f"Evals_{dataset}_{eps_str}.xlsx"
    ]
    ...
    if not file_path:
        return None, None, None, filenames_to_try[0]
    ...
    try:
        ...
        return pgd_df, trades_df, methods_df, loaded_filename
    except Exception as e:
        st.error(f"Error loading {loaded_filename}: {e}")
        return None, None, None, loaded_filename
...
def make_plot(pgd_df, trades_df, methods_df, eval_steps, eval_col, title, accuracy_thresh):
    ...
    if not df.empty:
        fig.add_trace(go.Scatter(
            x=df["clean"], y=df[eval_col], mode='markers',
            # Updated color to Teal (#17becf) for better visibility
            marker=dict(color="#17becf", size=12, symbol="diamond" if eval_steps == 10 else "diamond-open"),
            name=f"TRADES-{eval_steps}",
            text=[f"TRADES<br>β={b}<br>Train={tr}<br>Steps={ts}<br>Clean={c:.2f}<br>{eval_col}={r:.2f}"
                  for b, tr, ts, c, r in zip(df["beta"], df["Training"], df["PGD Steps"], df["clean"], df[eval_col])],
            hoverinfo="text"
...
# Load Data based on selection
pgd_df, trades_df, methods_df, expected_fname = load_data(selected_dataset, selected_epsilon)

if pgd_df is None:
    st.error(f"Data file not found for **{selected_dataset}** at epsilon **{selected_epsilon}**.")
    st.info(f"Make sure you have a file named like `data/{expected_fname}` (capitalization does not matter anymore).")
else:
    st.subheader(f"Results for {selected_dataset} (ε = {selected_epsilon})")
